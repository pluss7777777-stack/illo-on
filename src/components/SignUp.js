...
const SignUp = ({ isSns = false, onComplete }) => {
  ...
  const handleSubmit = async () => {
    try {
      if (isSns) {
        // 간편 회원가입 - 포트원 인증 후 받은 verificationId 전송
        const response = await axios.post('https://illoon.cloud/api/auth/signup/simple', {
        ...
        // 이메일 회원가입 - email, password, verificationId 전송
          ...
          {!isVerified ? (
            <div className="space-y-2.5">
              <input
                ...
                className="w-full h-[56px] px-5 bg-[#F2F4F7] rounded-[12px] outline-none"
              />
              <button onClick={handleVerifyRequest} className="w-full h-[56px] bg-[#2196F3] text-white font-bold rounded-[12px]">
                인증요청
              </button>
            </div>
          ) : (
