      ...
      <div style={{ maxWidth: 760, margin: '0 auto', padding: '32px 24px 120px' }}>
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 24 }}>
          <button
            ...
            style={{ display: 'flex', alignItems: 'center', gap: 6, border: `1.5px solid ${bookmarked ? '#2196F3' : '#E5E8EB'}`, borderRadius: 10, padding: '8px 16px', fontSize: 13, fontWeight: 600, color: bookmarked ? '#2196F3' : '#6B7684', background: bookmarked ? '#EEF6FF' : '#fff', cursor: 'pointer' }}>
        ...
        <div style={{ marginBottom: 32 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10 }}>
            {Array(4).fill(null).map((_, i) => (
              <div key={i} style={{ border: '1px solid #F2F4F7', borderRadius: 12, overflow: 'hidden', cursor: 'pointer' }}>
                <div style={{ height: 80, background: 'linear-gradient(135deg, #5BC8F5 0%, #2196F3 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.8)" strokeWidth="1.5" strokeLinecap="round">
                    <path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/>
                  </svg>
                </div>
                <div style={{ padding: '10px 12px' }}>
                  <div style={{ fontSize: 12, fontWeight: 600, color: '#191F28', lineHeight: 1.4, marginBottom: 4 }}>[위시스트] 너 김인직이지 (Product Designer / UIUX)</div>
                  <div style={{ fontSize: 11, color: '#ADB5BD' }}>충북 청주시 신입 - 경력 5년</div>
                </div>
              </div>
            ))}
          </div>
