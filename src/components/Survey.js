...
const Survey = () => {
  ...
  const submitSurvey = async () => {
    const user_id = localStorage.getItem('user_id') || 'test_user';; // user_id 가져오는 방식은 추후 수정
    setIsSubmitting(true);
    setSubmitError('');
    try {
      await axios.post(
        `https://port-0-illo-on-survey-mp2oa3fkd20c6566.sel3.cloudtype.app/api/v1/survey/${user_id}`,
        {
  ...
  const designTags = ['북디자인','앱디자인','웹디자인','시각디자인','공간디자인','건축디자인','게임디자인','영상디자인','공예디자인','로고디자인','패키지디자인','브랜드디자인','제품디자인','그래픽디자인','로고디자인','공공디자인'];
  ...
  // ── 공통 헤더: 캐릭터+프로그레스바 영역
  // 이미지 기준: 말풍선+캐릭터는 프로그레스바 시작점(약 20%) 위에 위치, 문구는 오른쪽
  ...
  // ── 하단 버튼: 이전 + 다음
  const BottomDual = ({ onPrev, onNext, nextActive }) => (
    <div className="w-full flex-shrink-0 flex flex-col items-center pb-10">
      <div className="w-full max-w-[1000px] h-[1px] bg-[#F2F4F7] mb-8 mx-auto"></div>
      ...
      <div className="flex flex-col w-full h-screen bg-white overflow-hidden font-sans">
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-4">
          <div className="w-full max-w-[600px]">
            ...
            <div>
              <p className="text-[14px] font-semibold text-[#4D5562] mb-2">입사년월</p>
              <div className="flex gap-3">
                <div className="flex-1"><Select value={joinYear} onChange={setJoinYear} placeholder="입사 연도 선택" options={yearOptions.map(y => `${y}년`)} /></div>
                <div className="flex-1"><Select value={leaveYear} onChange={setLeaveYear} placeholder="퇴사 연도 선택" options={yearOptions.map(y => `${y}년`)} /></div>
              </div>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="flex flex-col w-full h-screen bg-white overflow-hidden font-sans">
      ...
      <BottomDual onPrev={() => setStep(7)} onNext={() => {}} nextActive={true} />
    </div>
  );
};
