...
const OAuthCallback = ({ setView }) => {
  ...
  useEffect(() => {
    // URL 파라미터에서 token, isNewUser 꺼내기
    ...
    // isNewUser=true 면 이 컴포넌트에서 휴대폰 인증 화면 보여줌
  }, []);
  ...
  // isNewUser=true → 휴대폰 인증 화면
  if (isNewUser === null) return null; // 로딩 중
