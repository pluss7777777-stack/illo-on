  ...
  const FeaturedJobCard = ({ id, title, meta }) => {
    ...
    return (
      <div onMouseEnter={() => setHoveredCard(id)} onMouseLeave={() => setHoveredCard(null)} onClick={() => handleJobClick(id)}
        style={{ background: '#fff', borderRadius: 14, padding: '20px', cursor: 'pointer', boxShadow: isHovered ? '0 4px 20px rgba(0,0,0,0.12)' : '0 1px 6px rgba(0,0,0,0.06)', display: 'flex', flexDirection: 'column', minHeight: isHovered ? 280 : 160, transition: 'all 0.3s ease' }}>
        ...
        <div style={{ fontSize: 15, fontWeight: 700, color: '#191F28', lineHeight: 1.5, marginBottom: 8 }}>{title}</div>
        <div style={{ fontSize: 12, color: '#6B7684' }}>{meta}</div>
        {isHovered && (
          <button style={{ background: '#2196F3', color: '#fff', border: 'none', borderRadius: 10, padding: '12px 16px', fontSize: 13, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, marginTop: 'auto', paddingTop: 16 }}>
            지원하러 가기 <span>›</span>
          </button>
        )}
      </div>
  ...
  const SmallJobCard = ({ id, title, meta, platform }) => {
    ...
    return (
      <div
        ...
        style={{
          background: isHovered ? '#EEF6FF' : '#fff',
          borderRadius: 14,
          padding: '16px',
          cursor: 'pointer',
          boxShadow: isHovered ? '0 4px 20px rgba(0,0,0,0.12)' : '0 1px 6px rgba(0,0,0,0.06)',
        ...
        <div style={{ fontSize: 13, fontWeight: 600, color: '#191F28', lineHeight: 1.5, marginBottom: 6 }}>{title}</div>
        <div style={{ fontSize: 11, color: '#ADB5BD', marginBottom: 8 }}>{meta}</div>
        <button
          onClick={(e) => { e.stopPropagation(); handleJobClick(id); }}
          style={{
            width: '100%', padding: '10px 0', background: '#2196F3', color: '#fff',
      ...
      <div style={{ maxWidth: 1080, margin: '0 auto', padding: '24px 32px 80px' }}>
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, alignItems: 'start' }}>
            {['wanted','wanted','jobkorea','jobkorea','saramin','saramin'].map((platform, i) => (
              <SmallJobCard key={i} id={`illione-${i}`} title="[굴림] 프로덕트 디자이너 (Product Designer / UIUX)" meta="충북 청주시 신입 - 경력 5년" platform={platform} />
            ))}
          </div>
        </div>
