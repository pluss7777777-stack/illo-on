...
const Home = ({ setView, goToJobDetail, bookmarks, toggleBookmark, allJobs, setAllJobs }) => {
  ...
  const FeaturedJobCard = ({ id, title, meta }) => {
    ...
    return (
      <div onMouseEnter={() => setHoveredCard(id)} onMouseLeave={() => setHoveredCard(null)} onClick={() => handleJobClick(id)}
        style={{ background: isHovered ? 'linear-gradient(135deg, #2196F3 0%, #1565C0 100%)' : '#F0F7FF', borderRadius: 14, padding: '20px', cursor: 'pointer', border: isHovered ? '1px solid #2196F3' : '1px solid #E3F0FF', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', minHeight: 260, transform: isHovered ? 'scale(1.02)' : 'scale(1)', transition: 'all 0.2s ease', boxShadow: isHovered ? '0 8px 24px rgba(33,150,243,0.3)' : 'none' }}>
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
            <WantedLogo white={isHovered} />
            <div onClick={(e) => { e.stopPropagation(); toggleBookmark(id); }} style={{ cursor: 'pointer' }}><BookmarkIcon filled={!!bookmarks[id]} white={isHovered} /></div>
          </div>
        ...
        <button style={{ background: isHovered ? '#fff' : '#2196F3', color: isHovered ? '#2196F3' : '#fff', border: 'none', borderRadius: 10, padding: '10px 16px', fontSize: 13, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, marginTop: 16, transition: 'all 0.2s ease' }}>
          지원하러 가기 <span>›</span>
        </button>
      </div>
    );
  ...
  const SmallJobCard = ({ id, title, meta }) => {
    ...
    return (
      <div onMouseEnter={() => setHoveredCard(id)} onMouseLeave={() => setHoveredCard(null)} onClick={() => handleJobClick(id)}
        style={{ background: isHovered ? 'linear-gradient(135deg, #2196F3 0%, #1565C0 100%)' : '#fff', border: isHovered ? '1px solid #2196F3' : '1px solid #F2F4F7', borderRadius: 12, padding: '16px', cursor: 'pointer', boxShadow: isHovered ? '0 6px 20px rgba(33,150,243,0.25)' : '0 1px 4px rgba(0,0,0,0.04)', transform: isHovered ? 'scale(1.02)' : 'scale(1)', transition: 'all 0.2s ease' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
          <WantedLogo white={isHovered} />
          <div onClick={(e) => { e.stopPropagation(); toggleBookmark(id); }} style={{ cursor: 'pointer' }}><BookmarkIcon filled={!!bookmarks[id]} white={isHovered} /></div>
        </div>
        <div style={{ fontSize: 13, fontWeight: 600, color: isHovered ? '#fff' : '#191F28', lineHeight: 1.5, marginBottom: 6 }}>{title}</div>
        <div style={{ fontSize: 11, color: isHovered ? 'rgba(255,255,255,0.75)' : '#ADB5BD' }}>{meta}</div>
      </div>
    );
  };
      ...
      <div style={{ maxWidth: 1080, margin: '0 auto', padding: '24px 32px 80px' }}>
          ...
          <div style={{ flex: 1, minWidth: 0, background: '#F9FAFF', borderRadius: 18, padding: '20px 28px', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ flex: 1, position: 'relative', display: 'flex', alignItems: 'center' }}>
              ...
              <div ref={scrollRef} style={{ display: 'flex', gap: 14, overflowX: 'auto', scrollbarWidth: 'none', msOverflowStyle: 'none', width: '100%' }}>
                {recommendJobs.length > 0 ? recommendJobs.map((job) => (
                  <RecommendCard key={job.id} id={job.id} title={job.title} meta={`${job.company} · ${job.location}`} />
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          {regionJobsLoading ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10 }}>
              {regionJobs.map((job) => <SmallJobCard key={job.id} id={job.id} title={job.title} meta={`${job.company} · ${job.location}`} />)}
            </div>
          ) : (
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          {jobsLoading ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10 }}>
              {allJobs.map((job) => <SmallJobCard key={job.id} id={job.id} title={job.title} meta={`${job.company} · ${job.location}`} />)}
            </div>
          ) : (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10 }}>
              {Array(16).fill(null).map((_, i) => <SmallJobCard key={i} id={`all-${i}`} title="[글로벌] 프로덕트 디자이너 (Product Designer / UIUX)" meta="올투 천연수소 · 경력 3년↑" />)}
            </div>
          )}
          <div style={{ display: 'flex', justifyContent: 'center', marginTop: 20 }}>
            <button style={{ background: '#fff', border: '1px solid #E5E8EB', borderRadius: 10, padding: '10px 32px', fontSize: 13, fontWeight: 700, color: '#6B7684', cursor: 'pointer' }}>더보러가기 ›</button>
          </div>
        </div>
