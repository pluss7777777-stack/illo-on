...
const Home = ({ setView }) => {
  ...
  const FeaturedJobCard = ({ id, title, meta }) => {
    ...
    return (
      <div
        ...
        <button style={{
          background: isHovered ? '#fff' : '#2196F3',
          color: isHovered ? '#2196F3' : '#fff',
          border: 'none',
          borderRadius: 10,
          padding: '10px 16px',
          fontSize: 13,
  ...
  const RecommendCard = ({ id, title, meta }) => (
    <div style={{ minWidth: 260, maxWidth: 260, background: '#fff', borderRadius: 14, flexShrink: 0, boxShadow: '0 2px 10px rgba(33,150,243,0.1)', overflow: 'hidden', cursor: 'pointer', position: 'relative' }}>
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#fff', fontFamily: "'Pretendard', 'Apple SD Gothic Neo', sans-serif", width: '100%' }}>
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10 }}>
            {Array(8).fill(null).map((_, i) => (
              <SmallJobCard key={i} id={`region-${i}`} title="[글로벌] 프로덕트 디자이너 (Product Designer / UIUX)" meta="올투 천연수소 · 경력 3년↑" />
            ))}
          </div>
        </div>
      ...
      {/* 푸터 */}
      <div style={{ background: '#F2F4F7', borderTop: '1px solid #E5E8EB', padding: '32px' }}>
        <div style={{ maxWidth: 1080, margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
          <div>
            <img src="/logo2.png" alt="ILLO-ON" style={{ height: 24, marginBottom: 12 }} onError={e => e.target.style.display='none'} />
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
            ...
            <img src="/character6.png" alt="character" style={{ width: 100 }} onError={e => e.target.style.display='none'} />
          </div>
        </div>
      </div>
