...
const Home = ({ setView, goToJobDetail, bookmarks, toggleBookmark, allJobs, setAllJobs, goToSearch }) => {
      ...
      <div style={{ maxWidth: 1080, margin: '0 auto', padding: '24px 32px 80px' }}>
        ...
        <div style={{ display: 'flex', gap: 20, marginBottom: 24, alignItems: 'stretch' }}>
          <div style={{ width: 280, flexShrink: 0 }}>
            <div style={{ marginBottom: 10 }}>
              ...
              <div style={{ fontSize: 13, fontWeight: 600, color: '#191F28' }}>취업하러 이리오이소! 🔥</div>
            </div>
        ...
        <div ref={allJobsRef} style={{ marginBottom: 24 }}>
          ...
          {jobsLoading ? (
            ...
            <>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10 }}>
                {allJobs.map((job, i) => <SmallJobCard key={`${job.id}-${i}`} id={job.id} title={job.title} meta={`${job.company} · ${job.location}`} platform={['wanted','jobkorea','saramin'][i % 3]} />)}
              </div>
              {hasMore && (
