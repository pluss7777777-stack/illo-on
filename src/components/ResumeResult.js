...
const ResumeResult = ({ setView }) => {
  ...
  // 4개 영역 마름모꼴 (스킬 위, 경력 오른쪽, 경험/활동 아래, 기타스펙 왼쪽)
  const n = 4;
  const cx = 150, cy = 150, R = 100;
  const scores = [75, 80, 55, 60]; // 스킬, 경력, 경험/활동, 기타스펙
  const labels = [
      ...
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '0 16px 120px' }}>
              ...
              <div style={{ position: 'relative', height: 260 }}>
                ...
                <div style={{ position: 'absolute', bottom: 44, left: 60, display: 'flex', gap: 40, alignItems: 'flex-end' }}>
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                    ...
                    <div style={{
                      width: 70, height: `${barH(70)}px`,
                      background: '#2196F3', borderRadius: '35px 35px 0 0',
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                    ...
                    <div style={{
                      width: 70, height: `${barH(80)}px`,
                      background: '#D1D5DB', borderRadius: '35px 35px 0 0',
                      transition: 'height 1s cubic-bezier(0.34,1.56,0.64,1) 0.1s',
                      minHeight: 0,
                    }} />
                  </div>
                </div>
        ...
        <div style={{ background: '#EFF6FF', border: '1px solid #BFDBFE', borderRadius: 16, padding: '20px', marginBottom: 14 }}>
          ...
          {[
            {
              title: '1.직무의 적합도',
              content: '지원하는 공고의 직무기술서(JD)에서 요구하는 필수 기술과 핵심 역량이 이력서와 포트폴리오에 선명하게 드러나야 해요.\n단순히 경험을 나열하는 것이 아니라, 회사가 찾고 있는 \'문제 해결사\'의 모습과 본인의 과거 경험이 얼마나 일치하는지를 증명하는 것이 핵심이에요.'
            },
            {
              title: '2.경험성 · 구체성',
              content: '프로젝트 내에서 본인이 담당한 역할과 구체적인 행동, 그리고 그로 인해 창출된 결과를 명확히 기술해야 해요.\n특히 결과 부분에서는 막연한 표현 대신 정량적인 수치나 데이터(예: 속도 20% 개선, 사용자 유입 1.5배 증가 등)를 활용해 성과의 객관성을 확보하세요.'
            },
            {
              title: '3.문제해결능력 · 기술역량',
              content: 'IT 직무에서는 단순히 \'사용해 본 기술\'을 넘어, 특정 기술이나 스택을 선택한 타당한 근거가 제시되어야 해요.\n구현 과정에서 마주한 기술적 한계나 트러블슈팅 사례를 통해, 문제를 정의하고 논리적으로 해결해 나가는 개발자/디자이너로서의 사고 과정을 보여주세요.'
            },
            {
              title: '4.문서의 완성도',
              content: '아무리 좋은 내용이라도 가독성이 떨어지면 매력이 반감돼요. 올바른 맞춤법과 간결한 문장 구조를 유지하며, 불필요한 수식어는 걷어내고 핵심 위주로 작성해야 해요.\n전체적인 구조가 논리적이고 시각적으로 읽기 편하게 구성되었는지 점검이 필요해요.'
            },
            {
              title: '5.신뢰성 · 차별성 · 일관성',
              content: '이력서와 자기소개서, 포트폴리오의 내용이 서로 충돌하지 않고 하나의 일관된 직무 정체성을 향해야 해요.\n과장된 표현이나 정형화된 문구(복붙)를 지양하고, 본인만이 겪은 고유한 경험과 고민을 담아내어 서류 전체에 대한 신뢰도와 독창성을 높히세요.'
            },
          ].map((item, i, arr) => (
            ...
            {[
              {
                ...
                title: '언어 자격증 취득',
                desc: '디자인 실무 협업 시 소통 능력을\n증명할 수 있는 TOEIC Speaking이나\nOPIc 점수 확보를 추천해요.',
              },
              {
                src: '/qualifications.png',
                title: '디자인 자격증 취득',
                desc: 'GTQ 1급이나 컴퓨터그래픽스운용기능사 등\n공인된 자격증을 통해\n툴 활용 능력을 증명해 보세요.',
              },
              {
                src: '/premier.png',
                title: '디자인 공모전 수상',
                desc: '수상 경력은 \'기타 스펙\' 영역을 가장 강력하\n게 채워줄 수 있는 요소입니다.',
              },
            ].map((rec, i) => (
