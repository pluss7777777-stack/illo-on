...
const Resume = ({ setView }) => {
  ...
  // 포트폴리오: 업로드 완료 시 url(서버 응답 경로)을 함께 보관
  const [portfolios, setPortfolios] = useState([
    { id: 1, name: '포트폴리오', file: null, fileName: '김여주의_포트폴리오.pdf', url: null, uploading: false, error: null },
  ]);
  ...
  // 포트폴리오 파일 업로드: POST /api/n1/files/upload (multipart/form-data)
  // 응답: ["resumes/uuid-filename.pdf", ...] 형태의 경로 배열 -> 첫 번째 값을 url로 사용
  const uploadPortfolioFile = async (portfolioId, file) => {
    setPortfolios(prev => prev.map(p => p.id === portfolioId ? { ...p, uploading: true, error: null } : p));
    try {
      const token = localStorage.getItem('access_token');
      const formData = new FormData();
      formData.append('files', file);

      const res = await fetch('https://illoon.cloud/api/n1/files/upload', {
        method: 'POST',
      ...
      // 응답: ["resumes/uuid-filename.pdf", ...]
      const uploadedUrl = Array.isArray(data) ? data[0] : data?.url ?? data;

      setPortfolios(prev => prev.map(p => p.id === portfolioId
        ? { ...p, uploading: false, url: uploadedUrl, fileName: file.name }
        : p
      ));
    } catch (err) {
      console.error(err);
      setPortfolios(prev => prev.map(p => p.id === portfolioId
        ? { ...p, uploading: false, error: '업로드에 실패했어요. 다시 시도해주세요.' }
        : p
      ));
    }
  };

  // 이력서 생성 시 사용할 포트폴리오 JSON (url 필드에 업로드 응답 경로 매핑)
  const buildPortfolioPayload = () =>
    portfolios
      .filter(p => p.url) // 업로드 완료된 것만 포함
      .map(p => ({
        name: p.name,
        url: p.url, // 파일 업로드 API에서 받은 경로(만료 30분)
      }));
        ...
        <div style={{ background: '#F8FAFC', borderRadius: 20, width: '100%', maxWidth: 680, maxHeight: '90vh', overflow: 'hidden', display: 'flex', flexDirection: 'column', boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }}>
                ...
                {portfolios.filter(p => p.fileName).map(p => (
                  <div key={p.id} style={pCard}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={pTitle}>{p.name}</div>
                        <div style={{ fontSize: 11, color: '#6B7684' }}>{p.fileName}</div>
                      </div>
                      {p.url && (
                        <a href={p.url} target="_blank" rel="noopener noreferrer"
                          style={{ fontSize: 11, color: '#2196F3', fontWeight: 600, textDecoration: 'none', flexShrink: 0 }}>
                          열람하기
                        </a>
                      )}
                    </div>
          ...
          <div style={{ padding: '12px 20px', background: '#fff', borderTop: '1px solid #F2F4F7', display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
            ...
            <button onClick={() => {
              ...
              // 이력서 생성 시 portfolios 필드는 buildPortfolioPayload()로 매핑해서 백엔드 전송
              setView && setView('resume-result');
            }} style={{ padding: '10px 24px', background: '#2196F3', borderRadius: 10, fontSize: 13, fontWeight: 700, color: '#fff', border: 'none', cursor: 'pointer' }}>작성완료</button>
          </div>
        </div>
      </div>
    );
          ...
          <div key={portfolio.id} style={s.card(false)}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                {portfolio.uploading && (
                  <div style={{ fontSize: 12, color: '#2196F3', marginTop: 2 }}>업로드 중...</div>
                )}
                {!portfolio.uploading && portfolio.fileName && (
                  <div style={{ fontSize: 12, color: '#6B7684', marginTop: 2 }}>{portfolio.fileName}</div>
                )}
                {portfolio.error && (
                  <div style={{ fontSize: 12, color: '#F04438', marginTop: 2 }}>{portfolio.error}</div>
                )}
              </div>
              {/* 업로드 완료된 포트폴리오는 url로 즉시 열람 가능 (단, 30분 후 만료) */}
              {portfolio.url && (
                <a href={portfolio.url} target="_blank" rel="noopener noreferrer"
                  style={{ fontSize: 12, color: '#2196F3', fontWeight: 600, textDecoration: 'none', flexShrink: 0, marginRight: 4 }}>
                  열람
                </a>
              )}
              <button
        ...
        <label style={{ display: 'block', cursor: 'pointer' }}>
          <input
            ...
            onChange={(e) => {
              ...
              // 선택 즉시 서버로 업로드 (POST /api/n1/files/upload)
              uploadPortfolioFile(newId, file);
              e.target.value = '';
            }}
          />
        ...
        <div style={{ maxWidth: 720, margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            ...
            <button onClick={() => {
              const user_id = localStorage.getItem('user_id') || 'test_user';
              localStorage.setItem(`resume_done_${user_id}`, 'true');
              // 이력서 생성 API 호출 시 portfolios 필드는 buildPortfolioPayload()로 매핑해서 전송
              setView && setView('resume-result');
            }}
