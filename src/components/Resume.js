...
const Resume = ({ setView }) => {
  ...
  const [careers, setCareers] = useState([
    {
      ...
      tasks: [{
        ...
        description: '인바디 벤처사와 인바디 디자인팀 소속으로 근무하여 의료기기 제품의 UX/UI 디자인 업무를 수행하였습니다. 직관적보행 분석기, 밸런스스케 등 의료 헬스케어 기기의 사용자 인터페이스 및 사용자 경험 설계를 담당하였습니다.',
      }],
    }
  ]);
  ...
  const [awards, setAwards] = useState([
    { id: 1, name: '커뮤니케이션 디자인 국제 공모전', date: '2024.03', type: '수상', detail: '커뮤니케이션디자인 부문 옵션' },
  ...
  const s = {
    ...
    label: { fontSize: 12, color: '#6B7684', fontWeight: 400 },
    ...
    <div style={{ minHeight: '100vh', background: '#fff', fontFamily: "'Pretendard', 'Apple SD Gothic Neo', sans-serif" }}>
      ...
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '12px 16px 0' }}>
        <div style={{
          ...
          background: '#EFF6FF', border: '1px solid #BFDBFE', borderRadius: 10, padding: '9px 14px',
        }}>
      ...
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '0 16px 120px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginTop: 20 }}>
          <div style={{ display: 'flex', gap: 16, alignItems: 'flex-start', flex: 1 }}>
            {/* 프로필 이미지 */}
            <div style={{
              width: 88, height: 110, borderRadius: 12, background: '#EEF6FF',
                ...
                <div>
                  <div style={{ display: 'flex', gap: 8, marginBottom: 8, flexWrap: 'wrap' }}>
                    <input value={profileDraft.name} onChange={e => setProfileDraft({ ...profileDraft, name: e.target.value })}
                      placeholder="이름" style={{ ...s.input, width: 100 }} />
                    <select value={profileDraft.rank} onChange={e => setProfileDraft({ ...profileDraft, rank: e.target.value })}
                      style={{ ...s.input, width: 80 }}>
                      <option>신입</option><option>경력</option>
                    </select>
                  </div>
                  <div style={{ display: 'flex', gap: 8, marginBottom: 8, flexWrap: 'wrap' }}>
                    <input value={profileDraft.birth} onChange={e => setProfileDraft({ ...profileDraft, birth: e.target.value })}
                      placeholder="생년월일 (YYYY.MM.DD)" style={{ ...s.input, width: 160 }} />
                    <input value={profileDraft.age} onChange={e => setProfileDraft({ ...profileDraft, age: e.target.value })}
                      placeholder="나이" style={{ ...s.input, width: 60 }} />
                  </div>
                  <div style={{ display: 'flex', gap: 8, marginBottom: 10, flexWrap: 'wrap' }}>
                    <input value={profileDraft.phone} onChange={e => setProfileDraft({ ...profileDraft, phone: e.target.value })}
                      placeholder="전화번호" style={{ ...s.input, width: 140 }} />
                    <input value={profileDraft.email} onChange={e => setProfileDraft({ ...profileDraft, email: e.target.value })}
                      placeholder="이메일" style={{ ...s.input, width: 180 }} />
                  </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <div>
                  ...
                  <div style={{ ...s.metaText, marginTop: 2 }}>
                    {career.startDate} - {career.endDate} ({career.duration})&nbsp;&nbsp;|&nbsp;&nbsp;{career.type}&nbsp;&nbsp;|&nbsp;&nbsp;{career.role}&nbsp;&nbsp;{career.rank}
                  </div>
                </div>
              </div>
        ...
        <div style={{ display: 'flex', gap: 18, marginTop: 28 }}>
          <div style={{ flex: 1 }}>
            ...
            {awards.map(award => {
              ...
              const AwardIcon = () => {
                ...
                if (award.type === '자격증') return (
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#2196F3" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
                    <rect x="2" y="3" width="20" height="14" rx="2"/><path d="M8 21h8M12 17v4"/>
                    <path d="M9 10l2 2 4-4"/>
                  </svg>
                );
                if (award.type === '교육') return (
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#2196F3" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M22 10v6M2 10l10-5 10 5-10 5z"/><path d="M6 12v5c3 3 9 3 12 0v-5"/>
                  </svg>
                );
                    ...
                    <div style={{ marginTop: 10 }}>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 8 }}>
                        <div style={{ gridColumn: '1 / -1' }}>
                          <div style={{ fontSize: 11, color: '#6B7684', marginBottom: 3 }}>활동명</div>
                          <input value={award.name} onChange={e => setAwards(awards.map(a => a.id === award.id ? { ...a, name: e.target.value } : a))} style={s.input} />
                        </div>
                        <div>
                          <div style={{ fontSize: 11, color: '#6B7684', marginBottom: 3 }}>날짜</div>
                          <input value={award.date} onChange={e => setAwards(awards.map(a => a.id === award.id ? { ...a, date: e.target.value } : a))} placeholder="YYYY.MM" style={s.input} />
                        </div>
                        <div>
                          <div style={{ fontSize: 11, color: '#6B7684', marginBottom: 3 }}>타입</div>
                        ...
                        <div style={{ gridColumn: '1 / -1' }}>
                          <div style={{ fontSize: 11, color: '#6B7684', marginBottom: 3 }}>상세 내용</div>
                          <input value={award.detail} onChange={e => setAwards(awards.map(a => a.id === award.id ? { ...a, detail: e.target.value } : a))} style={s.input} />
                        </div>
                      </div>
            ...
            {/* 추가 버튼 */}
            <button
              onClick={() => setAwards([...awards, { id: Date.now(), name: '활동명', date: 'YYYY.MM', type: '기타', detail: '' }])}
              style={{ ...s.card(true), display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', cursor: 'pointer', textAlign: 'left' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                {/* ... 아이콘 */}
          ...
          <div style={{ width: 210, flexShrink: 0 }}>
            ...
            <button
              onClick={() => setLangs([...langs, { id: Date.now(), name: '시험명', date: 'YYYY.MM', type: '타입', detail: '' }])}
        ...
        <div style={s.card(false)}>
          {editingSelfIntro ? (
            <div>
              <textarea value={selfIntroDraft} onChange={e => setSelfIntroDraft(e.target.value)}
      ...
      <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: '#fff', borderTop: '1px solid #F2F4F7', padding: '12px 24px' }}>
        <div style={{ maxWidth: 720, margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            <button style={{ width: 90, height: 42, border: '1px solid #E5E8EB', borderRadius: 10, fontSize: 13, fontWeight: 600, color: '#6B7684', background: '#fff', cursor: 'pointer' }}>미리보기</button>
            <button onClick={() => setView && setView('resume-result')}
              style={{ width: 90, height: 42, background: '#2196F3', borderRadius: 10, fontSize: 13, fontWeight: 700, color: '#fff', border: 'none', cursor: 'pointer' }}>작성완료</button>
          </div>
        </div>
      </div>
