...
const Login = ({ setView }) => {
  ...
  // 2. 로그인 버튼 눌렀을 때 실행되는 함수
  const handleLogin = async (e) => {
    e.preventDefault();
    try {
      // 태민님이 준 서버 주소로 데이터 전송
      const response = await axios.post('http://15.164.247.11:8080/api/login', {
        email: email,
        password: password
      });
      ...
      if (response.status === 200) {
        ...
        setView('survey'); // 성공하면 설문조사 페이지로 이동
      }
    } catch (error) {
      console.error(error);
