...
const BookmarkPage = ({ setView, goToJobDetail }) => {
  ...
  const fetchScraps = async (page = 0) => {
    setScrapLoading(true);
    try {
      const token = localStorage.getItem('access_token');
      const res = await fetch(
        `https://illoon.cloud/api/scraps?page=${page}&size=${PAGE_SIZE}`,
        { headers: { 'Authorization': `Bearer ${token}` } }
      );
      if (!res.ok) throw new Error('스크랩 불러오기 실패');
      const data = await res.json();
      // 응답 구조: { content: [...], totalPages: N } 또는 배열 형태 둘 다 대응
      if (Array.isArray(data)) {
        setScrapJobs(data);
        setTotalPages(1);
      } else {
        setScrapJobs(data.content ?? data.jobs ?? []);
